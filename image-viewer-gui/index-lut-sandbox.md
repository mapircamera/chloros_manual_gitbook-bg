...
* **Прилагате нови индекси и цветови градиенти (LUT)**, за да визуализирате данните
* **Настройвате настройките за визуализация** интерактивно
...
Достъпът до индекс/LUT пясъчника се осъществява в **програмата за преглед на изображения** <img src="../.gitbook/assets/icon_image-viewer.JPG" alt="" data-size="line"> страничната лента:
...
4. Плъзнете цветните кръгчета на филтърния канал до местата във формулата за индекс по-долу
...
* **Изход**: RGB цвят (напр. ярко зелено)
* **Цел**: Да направи моделите по-лесни за разглеждане и интерпретиране
...
2. Наведете курсора на мишката върху нея, за да видите наличните предварителни настройки на градиента.
3. Изберете желания градиент.
4. Изображението **се актуализира незабавно** с новите цветове, когато отметката в полето „Индекс“ е поставена.
...
**Контролът на класовете** определя колко дискретни цветови стъпки се появяват в градиента:
...
4. Преходът се актуализира **в реално време** върху изображението

**Ефект върху визуализацията:**

* **По-малко класове** (3-5): Създава отделни зони, опростена класификация, по-лесно разграничаване на категориите
* **Средни класове** (6-10): Балансиран подход, подходящ за повечето приложения
* **Повече класове** (15-20): Плавни преходи, подробни вариации, фотографски вид
...
**Пример за настройки на диапазона NDVI:**

* **Пълен диапазон**: `-1.0` до `1.0` (показва всички възможни стойности)
* **Фокусиран върху растителността**: `0.2` до `0.9` (изключва голата почва и водата)
* **Само здрава растителност**: `0.5` до `0.9` (подчертава само жизнените растения)
* **Откриване на стрес**: `0.2` до `0.5` (подчертаване на проблемните области)
* **Персонализиран диапазон**: Настройка въз основа на наблюдаваните пикселни стойности
...
* Пиксели **над максимума** → показват се с **последния цвят** в градиента (например зелен)
...
* Само пикселите **в диапазона** показват цветови градиент
...
* **Пример за употреба**: Деликатно подчертаване, запазване на контекста при акцентиране на областите от интерес
* **Пример**: Подчертаване с цвят на стресираната растителност (NDVI 0,3-0,5), като здравите области се показват в сиво

#### 4. Оригинален фон

* Пикселите **извън обхвата** се показват като **оригинално мултиспектрално изображение**
* Пикселите **в рамките на обхвата** показват **цветови градиент**
...
За пълен контрол над визуализацията можете да създадете **персонализирани цветови градиенти**, като редактирате отделни цветови спирки.
...
* **Цветове на марката**: Съчетайте цветовата палитра на вашата организация за отчетите
...
7. **Усъвършенстване на цветовете** - Персонализирайте градиента, ако е необходимо, за специфично подчертаване
8. **Финализиране на настройките** - Документирайте настройките и копирайте в „Настройки на проекта“ за обработка на експорта
...
**Използване на пикселни стойности за настройка на LUT диапазони:**

След като проверите пикселните стойности, настройте съответно минималната/максималната стойност на LUT:

**Примерна ситуация:**

* **Наблюдение**: Стойности на почвата = 0,05-0,25, Стресирана = 0,25-0,50, Здрава = 0,50-0,85
* **Цел**: Визуализиране само на състоянието на растенията (изключване на почвата)
...
4. **Дефинирайте формулата си**, като използвате променливи на лентата:
   * Имена на лентите: `NIR`, `Red`, `Green`, `Blue`, `RedEdge` и др.
